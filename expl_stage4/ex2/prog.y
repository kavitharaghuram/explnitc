...
void Install(char* name, int type, int size, int isArray, int rows, int cols, int isPointer);
void printGsymbol();
int currentType;
tnode* root;
FILE* target_file;
...
Varlist
    : Varlist ',' ID '[' NUM ']' {
        if($5->val<=0){
            ...
            Install($3->varname, currentType, $5->val, 1, $5->val, 1, 0);
        }
    | Varlist ',' ID '[' NUM ']' '[' NUM ']' {
            if($5->val<=0 || $8->val<=0){
            ...
            Install($3->varname, currentType, $5->val*$8->val, 1, $5->val, $8->val, 0);
        }
    | Varlist ',' ID {
            Install($3->varname, currentType, 1, 0, 1, 1, 0);
        }
    | Varlist ',' '*' ID {
            Install($4->varname, currentType, 1, 0, 1, 1, 1);
        }
    | '*' ID {
            Install($2->varname, currentType, 1, 0, 1, 1, 1);
        }
    | ID '[' NUM ']' '[' NUM ']' {
            if($3->val<=0 || $6->val<=0){
                printf("Array dimensions must be positive\n");
            ...
            Install($1->varname, currentType, $3->val*$6->val, 1, $3->val, $6->val, 0);
        }
    | ID '[' NUM ']' {
        if($3->val<=0){
            printf("Array size must be positive\n");
            ...
            Install($1->varname, currentType, $3->val, 1, $3->val, 1, 0);
        }
    | ID { Install ($1->varname, currentType, 1, 0, 1, 1, 0); }
    ;
Slist
...
void printTree(tnode* t, int level)
{
    ...
    switch(t->nodetype)
    {
        ...
        case NODE_ID:
            printf("ID(%s) [type=%d, pointer=%d, binding=%d]\n",
                   t->varname,
                   t->type,
                   t->isPointer,
                   t->Gentry->binding);
